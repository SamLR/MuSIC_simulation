...
G4LogicalVolume* scintWLSSlab::ScintSlab_log=NULL;

scintWLSSlab::scintWLSSlab(G4RotationMatrix *pRot,
		 ...
		 "Slab",pMotherLogical,pMany,pCopyNo),constructor(c)
{
  ...
  if(!ScintSlab_log || updated){
    ...
    for(G4int i=0;i<nfibers;i++){
      ...
      new scintWLSFiber(rm,G4ThreeVector(0.,Y,0.),ScintSlab_log,false,0,
		      constructor);
    }
